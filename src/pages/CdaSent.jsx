...
// Badge for comparison string fields (match / mismatch / null)
...
// ── Component ─────────────────────────────────────────────────────────────────
function CdaSent() {
    const [filter, setFilter] = useState('all'); // 'all' | 'mismatch'
    ...
    const filteredData = useMemo(() => {
        ...
        return data.filter(row =>
            ...
            (row.skyslopefileid || '').toLowerCase().includes(q)
        );
    }, [data, searchQuery]);
                ...
                <div className="search-filter-bar">
                    <div className="search-input-wrapper">
                        ...
                        <input
                            ...
                            placeholder="Search by Transaction ID, SkySlope File ID, or Property Address…"
                            value={searchQuery}
                                ...
                                <tbody>
                                    {paginatedData.map((row, i) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <td className="cell-guid" title={row.transaction_id}>{row.transaction_id?.slice(0, 18) || '—'}…</td>
                                                <td className="cell-address">{row.property_address || '—'}</td>
                                                <td style={{ maxWidth: '180px', whiteSpace: 'normal', lineHeight: '1.4' }}>
                                                    {row.tags
